...
// ============================================================
// PARAGRAPHE — Révélation mot par mot
// ============================================================
...
function ParcoursCard({
  ...
  compact = false,
  padding = "p-8",
  revealTitleSize = "text-xl xl:text-[2.5vw]",
  revealYearSize = "text-sm xl:text-[1.2vw]",
  revealDescSize = "text-sm xl:text-[1.5vw]",
  revealPadding = "p-6",
  revealGap = "gap-5 xl:gap-8",
  iconClass = "w-12 h-12 xl:w-[2.8vw] xl:h-[2.8vw]",
}) {
  ...
  return (
    <motion.div
      ...
      className={`relative bg-[#0c0c0c] ${padding} overflow-hidden rounded-md flex flex-col justify-center select-none ${
...
function TechKey({
  ...
  return (
    <motion.div
      ...
      className={`tech-key relative flex flex-col items-center justify-center rounded-lg border-2 bg-[#1a1a1a] select-none ${
...
function TechCard({
  ...
  useEffect(() => {
    ...
    const handleOutsideClick = (e) => {
      if (!e.target.closest(".tech-key")) setActiveKey(null);
    };
    ...
    <div className={`relative z-0 flex flex-wrap ${gridContainerClass} ${gapClass}`}>
      {techs.map((tech) => (
        <TechKey
          ...
          tech={tech}
          mode={mode}
          isActive={activeKey === tech.name}
          onClick={() => setActiveKey(activeKey === tech.name ? null : tech.name)}
          sizeClass={keySizeClass}
          nameSize={keyNameSize}
        />
      ))}
    </div>
  );
  ...
  return (
    <div className={`relative flex flex-col ${className}`}>
      <Star 
        className="absolute top-2 right-2 md:top-4 md:right-4 w-10 h-10 md:w-16 md:h-16 text-yellow-400 rotate-[15deg] z-40 drop-shadow-lg pointer-events-none" 
        fill="#facc15" 
        strokeWidth={1.5} 
      />
...
function IntroOverlay({ startAnimation }) {
  ...
  const phrases = [
    "Je suis qui ?", "Who am I?", "¿Quién soy?", "?? ??? ?", "Wer bin ich?", "Chi sono?",
    "Qui je suis ?", "Je suis qui ?", "Who am I?", "Chi sono?",
  ];
              ...
              {stacked && phase >= 1 && (
                <div
                  className={`absolute -bottom-3 -right-3 w-full h-full rounded-xl border-2 transition-all duration-300 ${
                    phase === 1 ? "border-yellow-400/25 border-dashed" : "border-yellow-400/60 bg-yellow-400/30"
                  }`}
                />
              )}
              <div
                className={`relative w-full h-full flex items-center justify-center rounded-xl border-2 transition-all duration-300 ${
                  phase === 0
                    ...
                    : phase === 1
                    ? "border-yellow-400/50 border-dashed bg-transparent"
                    : "border-yellow-400 border-solid bg-yellow-400"
                }`}
              >
                {phase === 1 && (
                  <span
                ...
                {phase === 2 && (
                  <motion.span
                    ...
                    className="font-black text-black text-4xl md:text-6xl"
...
export default function About() {
  ...
  // ============================================================
  // PLAQUE DESKTOP — Range of motion et Dimensions importés de Hero
  // ============================================================
  ...
  const handleMouseMove = (e) => {
    ...
    // Contrainte calculée pour ne jamais chevaucher les sections de droite (Parcours/Skills)
  ...
  // ============================================================
  // PLAQUE MOBILE — Range of motion importé de Hero, restreint au top
  // ============================================================
  ...
  const handlePointerMoveMobile = (e) => {
    ...
    // Strictement limitée pour ne bouger qu'à l'intérieur du "mobileTopRef" (le top mi-côté)
  ...
  return (
    <div ref={sectionRef} className="w-full h-full">
      {/* ============================================================
          DESKTOP ET TABLETTE (>= md) 
          ============================================================ */}
      <section
        className="hidden md:flex relative w-full h-screen bg-[#080808] overflow-hidden font-cartoon text-white flex-row px-4 py-4 gap-2"
      >
        <AnimatePresence>
        ...
        >
          ...
          <motion.p
            ...
            className="relative z-10 text-4xl lg:text-5xl xl:text-[3.2vw] uppercase tracking-wide text-white/70 mb-2 mt-6"
          >
            {smallIntro}
          </motion.p>
          ...
          <div className="relative inline-block select-none">
            ...
            {/* PLAQUE DESKTOP: Dimensions Hero appliquées */}
            <motion.div
        ...
        <div className="flex flex-col flex-1 h-full py-2 gap-4 z-10">
          ...
          <TechCard
            className="flex-[0.55]"
            mode="hover"
            keySizeClass="w-14 h-14 xl:w-[4.4vw] xl:h-[4.4vw]"
            keyNameSize="text-[9px] xl:text-[0.6vw]"
          />
        </div>
      </section>

      {/* ============================================================
          MOBILE UNIQUEMENT (< md, soit les téléphones)
          ============================================================ */}
      <section
        className="flex md:hidden relative w-full h-[100dvh] bg-[#080808] overflow-hidden font-cartoon text-white flex-col px-4 py-3 gap-2"
      >
        <AnimatePresence>
        ...
        <div
          ...
          className="relative flex-shrink-0 flex flex-col mt-3 justify-center z-10 pt-8 pb-2"
        >
          {/* PLAQUE MOBILE: Placée de façon absolue dans le container top, dimensions Hero */}
          <motion.div
            ...
            className="absolute z-20 top-[35%] left-[24%] w-[52%] h-[25%] bg-white rounded-[18px] mix-blend-difference pointer-events-none"
          />
          ...
          <motion.p
            ...
            className="relative z-10 text-[6vw] sm:text-[5vw] uppercase tracking-wide text-white/70"
          >
            {smallIntro}
          </motion.p>
          ...
          <RevealParagraph
            segments={introSegments}
            isVisible={!showOverlay}
            className="relative z-10 text-[4.5vw] sm:text-[3.6vw] leading-snug normal-case"
          />
        </div>

        {/* Collage empilé (sans scroll) */}
        <div className="relative flex-1 min-h-0 z-10 mt-1">
          <motion.div
            initial={{ opacity: 0, rotate: 5, y: 20 }}
            animate={!showOverlay ? { opacity: 1, rotate: 2, y: 0 } : {}}
            transition={{ type: "spring", stiffness: 60, damping: 14, delay: 0.3 }}
            className="absolute bottom-0 right-0 w-[92%] sm:w-[86%] h-[86%] sm:h-[84%] z-10"
          >
            <TechCard
              className="w-full h-full bg-[#0c0c0c] border-2 border-dashed border-white/10 rounded-xl px-3 pt-4 pb-3 justify-between"
              mode="click"
              titlePosition="bottom"
              showTitle
              titleSize="text-[5.5vw] sm:text-[4vw]"
              titleContainerClass="text-right w-full mt-2"
              gridContainerClass="justify-end mt-[30%] content-center"
              keySizeClass="w-[12.5vw] h-[12.5vw] sm:w-[10vw] sm:h-[10vw]"
          ...
          <motion.div
            ...
            animate={!showOverlay ? { opacity: 1, rotate: -3, y: 0 } : {}}
            transition={{ type: "spring", stiffness: 60, damping: 14, delay: 0.45 }}
            className="absolute top-0 left-0 w-[62%] sm:w-[45%] h-[38%] sm:h-[35%] z-20 shadow-[8px_8px_0_rgba(0,0,0,.5)]"
          >
            <ParcoursCard
              ...
              compact
              padding="p-4"
              titleSize="text-[5.5vw] sm:text-[4.5vw]"
              revealTitleSize="text-sm sm:text-base"
              revealYearSize="text-[10px] sm:text-xs"
              revealDescSize="text-xs"
              revealPadding="p-3"
              revealGap="gap-1 sm:gap-2"
              iconClass="w-6 h-6 sm:w-8 sm:h-8"
            />
          </motion.div>
        </div>
      </section>
